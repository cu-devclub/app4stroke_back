...
import { BadRequestError } from "../errorHandler/httpError/httpError";

const url = '/api/predict_prob';
const method = 'post';
const header = {
...
const PredictScore = async(req: Request, res: Response) => {
  try {
    const data = await predictScoreRequestDataSchema.validateAsync(req.params);
  } catch (error) {
    const ErrorHandle = new BadRequestError('Error');
    res.status()
  }
